...
- **Framework upgrade — Astro 4→6 + Node 20→24 (own focused session).** Deferred deliberately: it's two majors stacked on a Tailwind 3→4 migration on a live site, needs in-browser verification of the Vercel preview before cutover. Known breaking points: (1) `output: 'hybrid'` → `'static'` in astro.config; (2) content collections legacy `.slug` → Content Layer / `.id`; (3) `@astrojs/tailwind` is deprecated → Tailwind v4 via `@tailwindcss/vite`, custom colors move to CSS `@theme` (visual-regression risk); (4) `@astrojs/vercel/serverless` import path changes in v8; (5) verify Keystatic + Astro 6 compatibility (possible hard blocker — check before starting). Node 20 is EOL as of Apr 2026, so this isn't optional forever. The preview-noindex gate now in place makes reviewing a preview deploy safe. **This bundles the remaining `npm audit fix --force` items** — they clear when the framework pins bump.
